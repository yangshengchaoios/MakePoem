...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated title:(NSString *)title{
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
	hud.labelText = title;
	[view addSubview:hud];
	[hud show:animated];
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view withTitle:(NSString *)title{
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
	hud.labelText = title;
	[view addSubview:hud];
	[hud show:YES];
...
+ (MBProgressHUD *)error:(UIView *)view{
    for (UIView *view_ in view.subviews) {
        if([view_ isKindOfClass:[self class]]){
...
+ (void)hideHUDForView:(UIView *)view withTitle:(NSString *)title{
...
- (void)autoHideAfterTimeInterval:(NSTimeInterval)ti title:(NSString *)title obj:(UIView *)obj{
    [self initWithView:obj];
    [obj addSubview:self];
    [self show:YES];
    [self performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:ti];
    [self release];
}

- (void)removeAfterTimeInterval:(NSTimeInterval)ti title:(NSString *)finishTitle{
    self.labelText = finishTitle;
    ...
    [self performSelector:@selector(show:) withObject:[NSNumber numberWithBool:NO] afterDelay:ti];
    [self release];
}
